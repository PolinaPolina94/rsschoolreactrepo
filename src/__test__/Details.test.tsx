...
describe('Details card tests', () => {
  ...
  it('Render details of card after click on it', async () => {
    render(
      <BrowserRouter>
        <ListItems items={objectState.items} />
      </BrowserRouter>
    ...
    waitFor(() => {
      ...
      expect(screen.getByRole('person'));
      expect(screen.getByText(/IDENTIFICATION-NUMBER/));
    });
  });
  it('Render details card', async () => {
    render(
      <MemoryRouter initialEntries={['/details/1']}>
        <Person />
      </MemoryRouter>
  ...
  it('Close details card after click close button', async () => {
    render(
      <MemoryRouter initialEntries={['/details/1']}>
        <Person />
      </MemoryRouter>
